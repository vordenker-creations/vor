...
class ActivityHeatmapWidget(QFrame):
    def __init__(self, parent=None):
        ...
        t_h = QHBoxLayout()
        t = QLabel("Productivity Intensity Heatmap")
        ...
        import random
        # 24 weeks x 7 days for a dense dashboard look
        for week in range(24):
            for day in range(7):
                intensity = random.randint(0, 4) if random.random() > 0.2 else 0
        ...
        leg_h = QHBoxLayout()
        leg_h.addStretch()
        leg_h.addWidget(QLabel("Less Intensity", styleSheet="font-size: 11px; color: #64748B; border: none;"))
        for i in range(5):
            leg_h.addWidget(HeatmapCell(i))
        leg_h.addWidget(QLabel("More Intensity", styleSheet="font-size: 11px; color: #64748B; border: none;"))
        l.addLayout(leg_h)
