...
class AnalyticsMetricCard(QFrame):
    def __init__(self, title, value, trend, subtext, color="#38BDF8", parent=None):
        ...
        # Value
        v_lbl = QLabel(value)
        v_lbl.setStyleSheet("font-size: 28px; font-weight: 800; color: #0F172A; border: none;")
        l.addWidget(v_lbl)
...
class ProgressMetricCard(QFrame):
    def __init__(self, title, val, color="#38BDF8", parent=None):
        ...
        v_h = QHBoxLayout()
        v_lbl = QLabel(f"{val}%")
        v_lbl.setStyleSheet(f"font-size: 22px; font-weight: 800; color: {color}; border: none;")
        v_h.addWidget(v_lbl)
        v_h.addStretch()
        l.addLayout(v_h)
