...
class WorkflowInsightCard(QFrame):
...
class AIWorkflowPanel(QFrame):
    ...
    def setup_ui(self):
        ...
        # 1. AI Workspace Insights
        self._setup_workspace_insights(cl)

        # 2. Workspace Analytics
        self._setup_workspace_analytics(cl)
    ...
    def _setup_workspace_insights(self, layout):
        sec = QVBoxLayout(); sec.setSpacing(16)
        lbl = QLabel("AI WORKSPACE INSIGHTS")
        lbl.setStyleSheet("font-size: 11px; font-weight: 800; color: #94A3B8; letter-spacing: 1px; border: none;")
        sec.addWidget(lbl)
        
        sec.addWidget(WorkflowInsightCard("Efficiency Boost", "Enabling 'Auto-Collapse Sidebar' could increase your visual focus by 15% during deep work."))
        sec.addWidget(WorkflowInsightCard("Workflow Detection", "You frequently switch to the Job Portal after resume edits. Consider linking these workflows.", "#F59E0B"))
        layout.addLayout(sec)

    def _setup_workspace_analytics(self, layout):
        sec = QVBoxLayout(); sec.setSpacing(12)
        lbl = QLabel("WORKSPACE ANALYTICS")
        lbl.setStyleSheet("font-size: 11px; font-weight: 800; color: #94A3B8; letter-spacing: 1px; border: none;")
        sec.addWidget(lbl)
        ...
        bl.addWidget(QLabel("Productivity Score: Elite", styleSheet="color: #10B981; font-weight: 800; font-size: 12px;"))
        bl.addWidget(QLabel("Your interaction speed has improved by 24% with the latest layout optimization.", styleSheet="color: #94A3B8; font-size: 11px;"))
        sec.addWidget(box)
        layout.addLayout(sec)

    def _setup_actions(self):
        al = QVBoxLayout(); al.setSpacing(10)
        btn_opt = QPushButton("Optimize Workspace")
        ...
        btn_reset = QPushButton("Reset Behaviors")
        ...
        al.addWidget(btn_opt); al.addWidget(btn_reset)
