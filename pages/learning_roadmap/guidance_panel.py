...
class GuidancePanel(QFrame):
    ...
    def setup_ui(self):
        ...
        # 2. Learning Velocity
        cl.addLayout(self._setup_velocity())

        # 3. Career Predictions
    ...
    def _setup_insights(self):
        sec = QVBoxLayout()
        sec.setSpacing(16)
        lbl = QLabel("AI LEARNING INSIGHTS")
        lbl.setStyleSheet("font-size: 11px; font-weight: 800; color: #94A3B8; letter-spacing: 1px; border: none;")
        sec.addWidget(lbl)
        
        sec.addWidget(AIInsightCard("Weak Skill Detected", "Your progress in 'Algorithms' has slowed. Try the 'Dynamic Programming' mini-course.", "#EF4444"))
        sec.addWidget(AIInsightCard("Roadmap Optimization", "Based on your 'Senior AI' goal, we recommend switching 'NLP' priority to 'High'."))
        return sec

    def _setup_velocity(self):
        sec = QVBoxLayout()
        sec.setSpacing(12)
        lbl = QLabel("LEARNING VELOCITY")
        lbl.setStyleSheet("font-size: 11px; font-weight: 800; color: #94A3B8; letter-spacing: 1px; border: none;")
        sec.addWidget(lbl)
        ...
        add_prog("Consistency", 92)
        add_prog("Retention Rate", 78)
        sec.addWidget(card)
        return sec
    ...
    def _setup_career(self):
        sec = QVBoxLayout()
        sec.setSpacing(16)
        lbl = QLabel("CAREER PREDICTIONS")
        lbl.setStyleSheet("font-size: 11px; font-weight: 800; color: #94A3B8; letter-spacing: 1px; border: none;")
        sec.addWidget(lbl)
        
        role = QFrame()
        role.setStyleSheet("background: #0F172A; border-radius: 16px; padding: 16px;")
        rl = QVBoxLayout(role)
        rl.addWidget(QLabel("Target Role: ML Lead", styleSheet="color: #38BDF8; font-weight: 800; font-size: 13px;"))
        rl.addWidget(QLabel("Match Probability: 84%", styleSheet="color: white; font-size: 11px; font-weight: 500;"))
        sec.addWidget(role)
        return sec
    ...
    def _setup_actions(self):
        ...
        btn_gen = QPushButton("✨ Generate AI Roadmap")
        ...
        btn_opt = QPushButton("Optimize Learning Plan")
